...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-8">
            <div class="mb-8">
                <h1 class="font-nav text-2xl font-bold text-gray-900">Editar Producto</h1>
                <p class="text-sm text-gray-500 mt-1">Complete los datos para actualizar el producto en el sistema.</p>
            </div>

            <form action="{{ route('product.update', $product->product_id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <select id="productionline" name="production_line_id" 
                                ...
                                @foreach ($productionLines as $line)
                                    <option value="{{ $line->production_line_id }}" 
                                        {{ old('production_line_id', $product->production_line_id) == $line->production_line_id ? 'selected' : '' }}>
                                        {{ $line->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="picture" id="picture" placeholder="Ej. https://example.com/product-image.jpg" 
                            value="{{ old('picture', $product->picture) }}" required
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="flex items-center space-x-6 py-2">
                            <label class="inline-flex items-center cursor-pointer group">
                                ...
                                {{ old('isactive', $product->isactive) == 'Y' ? 'checked' : '' }}
                            ...
                            <label class="inline-flex items-center cursor-pointer group">
                                ...
                                {{ old('isactive', $product->isactive) == 'N' ? 'checked' : '' }}
